...
def mainFunc(numOfTrain, numOfTest, virusProb, boardSize, maxDist):
    ...
    while count < numOfTrain + numOfTest: # numOfTrain + numOfTest개의 데이터
        try:
            ...
            assert(len(nodeNo) == len(hop1Noise) and len(hop1Noise) == len(hop1Max) and len(hop1Noise) == len(hop2AvgNoise) and len(hop1Noise) == len(hop2AvgMax))
    ...
    for j in range(len(hop1Noise_)):
        hop1NoiseZ_.append(ex.Zvalue(hop1Noise_, hop1Noise_[j]))
        hop1MaxZ_.append(ex.Zvalue(hop1Max_, hop1Max_[j]))
        hop2AvgNoiseZ_.append(ex.Zvalue(hop2AvgNoise_, hop2AvgNoise_[j]))
        hop2AvgMaxZ_.append(ex.Zvalue(hop2AvgMax_, hop2AvgMax_[j]))
    ...
    for i in range(len(hop1Noise_)):
        toSave_ = str(count_[i]) + '\t' + str(nodeNo_[i]) + '\t' + str(round(hop1Noise_[i], 4)) + '\t' + str(round(hop2AvgNoise_[i], 4)) + '\t'
        toSave_ += str(round(hop1NoiseZ_[i], 10)) + '\t' + str(round(hop2AvgNoiseZ_[i], 10)) + '\t'
        toSave_ += str(round(hop1Max_[i], 4)) + '\t' + str(round(hop2AvgMax_[i], 4)) + '\t'
        toSave_ += str(round(hop1MaxZ_[i], 10)) + '\t' + str(round(hop2AvgMaxZ_[i], 10)) + '\t' + str(isMal_[i])
        print(toSave_)
