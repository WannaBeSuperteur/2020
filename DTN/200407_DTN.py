...
def trainAndTest(numOfTrain, numOfTest, virusProb, boardSize, maxDist, deviceName): # 학습 데이터를 읽어서 배열에 저장 및 학습
    ...
    for epoch in range(len(epochList.split(' '))):
        ...
        # 신경망 생성
        NN = [tf.keras.layers.Flatten(input_shape=(6,)),
        ...
        # 입력 데이터: hop1NoiseZ_, hop1MaxZ_, hop2AvgNoiseZ_, hop2AvgMaxZ_, hop1NeisZ_, hop2AvgNeisZ_(6~11) / 출력 데이터: isMal_(12)
        inputs = []
        outputs = []
        for i in range(lenTrain):
            inputs.append([mainData_train[6][i], mainData_train[7][i], mainData_train[8][i], mainData_train[9][i], mainData_train[10][i], mainData_train[11][i]])
        ...
        for i in range(lenTest):
            testInputs.append([mainData_test[6][i], mainData_test[7][i], mainData_test[8][i], mainData_test[9][i], mainData_test[10][i], mainData_test[11][i]])
