...
def SVM(data, classes, newData, displayChart):
    ...
    for i in range(len(newData)):
        ...
        if displayChart: # thisNewData[2] : actual class
            if predictedClass == 0 and thisNewData[2] == 0: plt.plot(x, y, 'b.')
            elif predictedClass == 1 and thisNewData[2] == 0: plt.plot(x, y, 'rx')
            elif predictedClass == 0 and thisNewData[2] == 1: plt.plot(x, y, 'bx')
            elif predictedClass == 1 and thisNewData[2] == 1: plt.plot(x, y, 'r.')

        # check if prediction is correct
        if predictedClass == thisNewData[2]: correct += 1

    print('correct rate: ' + str(round(100 * correct / len(newData), 2)) + '%')
    if displayChart:
