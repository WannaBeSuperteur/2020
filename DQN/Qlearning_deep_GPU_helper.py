...
def maxQvalue(screen, isFile, model0, model1, last0, last1, option):
    ...
    if isFile:
        if option % 3 == 0 or option % 3 == 1: Qvalues = Qlearning_deep_GPU.modelOutput(model0, [screen])
        elif option % 3 == 2: Qvalues = Qlearning_deep_GPU.modelOutput(model1, [screen])
    else: return 0
...
def getQvalues(screen, isFile, model0, model1, last0, last1, func0, actions, option):
    ...
    if isFile:
        if option % 3 == 0 or option % 3 == 1:
            Qvalues = Qlearning_deep_GPU.modelOutput(model0, [screen]) # 이미 flatten된 screen임
...
def decideAction(option, screen, screenCopy, point, additional, isFile, model0, model1, last0, last1, gameNo,
    ...
    if isFile:
        ...
        if option % 3 == 0: Qvalues = Qlearning_deep_GPU.modelOutput(model0, [screenCopy])
        ...
        # Double DQN
            ...
            for action in range(actions): # 각 행동에 대하여
                ...
                Qvalues0 = Qlearning_deep_GPU.modelOutput(model0, [newScreenCopy])
                
                # model 1에 대한 Q value (reward가 가장 큰 action만 one-hot으로 1임)
                # Q(s_(t+1), argmax(a)Q(s_(t+1), a))에서 상태가 s_(t+1)이므로 newScreen을 이용
                Qvalues1 = Qlearning_deep_GPU.modelOutput(model1, [newScreenCopy])
        ...
        # Dueling Network
        elif option % 3 == 2: Qvalues = Qlearning_deep_GPU.modelOutput(model1, [screenCopy])
