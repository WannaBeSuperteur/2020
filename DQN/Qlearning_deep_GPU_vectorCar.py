...
def feasible(screen, move, vector, point):
    ...
    if not Qlearning_deep_GPU_helper.exceed(screen, newPoint[0], newPoint[1]): return True
    else: return False
...
def getNextState(screen, action, vector):
    ...
    if Qlearning_deep_GPU_helper.exceed(screenCopy, point[0], point[1]): return None
...
if __name__ == '__main__':
    ...
    for i in range(games):
        ...
        while(1):
            ...
            screenCopy = Qlearning_deep_GPU_helper.logData(learningSize=learningSize, charSize=3, screen=screen, point=point,
            ...
            decision = Qlearning_deep_GPU_helper.decideAction(option, screen, screenCopy, point, vector, isFile,
        ...
        if len(states) < 1000: # 1000개 미만이면 모두 학습
            Qlearning_deep_GPU.deepQlearning(option, NN, op, 'mean_squared_error', states, outputs, Qdiffs, 25, 'vectorCar', [20, 25], [8, 'sigmoid', 'sigmoid'], [6, 'sigmoid', 'sigmoid'], False, False, True, deviceName)
        else: # 1000개 이상이면 마지막 1000개만 학습
            Qlearning_deep_GPU.deepQlearning(option, NN, op, 'mean_squared_error', states[len(states)-1000:], outputs[len(states)-1000:], Qdiffs[len(states)-1000:], 25, 'vectorCar', [20, 25], [8, 'sigmoid', 'sigmoid'], [6, 'sigmoid', 'sigmoid'], False, False, True, deviceName)
        isFile = True

        if option % 3 == 0 or option % 3 == 1: model0 = Qlearning_deep_GPU.deepLearningModel('vectorCar_0', False)
        if option % 3 == 1 or option % 3 == 2: model1 = Qlearning_deep_GPU.deepLearningModel('vectorCar_1', False)
