...
def deepQlearning(option, NN, op, loss, states, outputs, Qdiff, repeat, saveName, epoch, dense0, dense1, isPrint, dataPrint, modelPrint):
    ...
    if not Dueling: # Dueling Network는 따로 학습
        ...
        learning(model, [states], [outputs], saveName + '_0', epoch[0]) # 모델 저장은 이 함수에서 이루어짐
    ...
    if DoubleDQN:
        ...
        learning(model_, [states], [outputs_], saveName + '_1', epoch[1]) # 모델 저장은 이 함수에서 이루어짐
    ...
    if Dueling:
        ...
        learning(model, [states], [outputs], saveName + '_1', epoch[1]) # 모델 저장은 이 함수에서 이루어짐
...
# isPrint     : 관련 정보 출력 여부
def learnFrozenLake(fn, isStochastic, lrStochastic, lrDeepQ, isPrint):
    ...
    learning(model, [states], [outputs], 'FrozenLake', 4)    
...
if __name__ == '__main__':
    learnFrozenLake('map.txt', True, 0.8, 0.001, True)
