    ...
    for j in range(int(size*size/2)):
        ...
        for y in range(centerY-2, centerY+3):
            for x in range(centerX-2, centerX+3):
                if Qlearning_deep_GPU_helper.exceed(originalScreen, y, x): continue
...
if __name__ == '__main__':
    ...
    for i in range(games):
        ...
        if len(states) < 1000: # 1000개 미만이면 모두 학습
            Qlearning_deep_GPU.deepQlearning(option, NN, op, 'mean_squared_error', states, outputs, Qdiffs, 25, 'valueMaze', [20, 25], [8, 'sigmoid', 'sigmoid'], [6, 'sigmoid', 'sigmoid'], False, False, True, deviceName)
        ...
        if option % 3 == 0 or option % 3 == 1: model0 = Qlearning_deep_GPU.deepLearningModel('valueMaze_0', False)
        if option % 3 == 1 or option % 3 == 2: model1 = Qlearning_deep_GPU.deepLearningModel('valueMaze_1', False)
