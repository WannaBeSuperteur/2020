    ...
    while True:
        ...
        for epo in range(len(epoch)): # 각 epoch에 대하여
            for dro in range(len(dropout)): # 각 dropout에 대하여 실험
                ...
                while True:
                    try:                  
                        # 4. 모델 정의 (총 3개의 Neural Network)
                        NN0 = [tf.keras.layers.Reshape((size, size, 1), input_shape=(size*size,)),
                        ...
                        NN1 = [tf.keras.layers.Reshape((size, size, 1), input_shape=(size*size,)),
                               keras.layers.Conv2D(32, kernel_size=(3, 3), input_shape=(size, size, 1), activation='softplus'),
                               keras.layers.MaxPooling2D(pool_size=2),
                               keras.layers.Dropout(drop),
                               keras.layers.Conv2D(32, (3, 3), activation='softplus'),
                               keras.layers.Flatten(),
                               keras.layers.Dropout(drop),
                               keras.layers.Dense(40, activation='softplus'),
                               keras.layers.Dense(2, activation='sigmoid')]
                        NN2 = [tf.keras.layers.Reshape((size, size, 1), input_shape=(size*size,)),
                        ...
                        for i in range(numTest):
                            ...
                            optiY_test2 = Qlearning_deep_GPU_helper.invSigmoid(testOutput2Layer[0][0], size-1) # 테스트 결과로 나온 throughput 최대화 y좌표
                            optiX_test2 = Qlearning_deep_GPU_helper.invSigmoid(testOutput2Layer[0][1], size-1) # 테스트 결과로 나온 throughput 최대화 x좌표

                            # 출력값 중 거리가 가장 가까운 두 점의 x, y좌표의 평균, 즉 중점을 이용
                            dist01 = pow(optiY_test0 - optiY_test1, 2) + pow(optiX_test0 - optiX_test1, 2)
                            dist02 = pow(optiY_test0 - optiY_test2, 2) + pow(optiX_test0 - optiX_test2, 2)
                            dist12 = pow(optiY_test1 - optiY_test2, 2) + pow(optiX_test1 - optiX_test2, 2)

                            if dist01 <= dist02 and dist01 <= dist12: # NN0과 NN1에 의한 출력값 간의 거리가 가장 가까우면
                                optiY_test = int(round((optiY_test0 + optiY_test1) / 2.0, 0))
                                optiX_test = int(round((optiX_test0 + optiX_test1) / 2.0, 0))
                            elif dist02 <= dist01 and dist02 <= dist12: # NN0과 NN2에 의한 출력값 간의 거리가 가장 가까우면
                                optiY_test = int(round((optiY_test0 + optiY_test2) / 2.0, 0))
                                optiX_test = int(round((optiX_test0 + optiX_test2) / 2.0, 0))
                            elif dist12 <= dist01 and dist12 <= dist02: # NN1과 NN2에 의한 출력값 간의 거리가 가장 가까우면
                                optiY_test = int(round((optiY_test1 + optiY_test2) / 2.0, 0))
                                optiX_test = int(round((optiX_test1 + optiX_test2) / 2.0, 0))
                            ...
                            # 정답과 비교
                            print('test throughput for map ' + str(i) + ' [Y=' + str(optiY_test) + ' X=' + str(optiX_test) + ' HT=' + str(HAPtime) + '] among [Y='
                                  + str(int(round(optiY_test0, 0))) + ' X=' + str(int(round(optiX_test0, 0))) + ', Y='
                                  + str(int(round(optiY_test1, 0))) + ' X=' + str(int(round(optiX_test1, 0))) + ', Y='
                                  + str(int(round(optiY_test2, 0))) + ' X=' + str(int(round(optiX_test2, 0))) + '] : '
                    ...
                    # 평가 결과 저장
                    fSave = open('DL_WPCN_result_' + str(problemNo) + '.txt', 'w')
