                ...
                while True:
                    try:                  
                        ...
                        NN = [tf.keras.layers.Reshape((size, size, 1), input_shape=(size*size,)),
                                keras.layers.Conv2D(32, kernel_size=(3, 3), input_shape=(size, size, 1), activation=LR),
                                keras.layers.MaxPooling2D(pool_size=2),
                                keras.layers.Dropout(drop),
                                keras.layers.Conv2D(32, (3, 3), activation=LR),
                                keras.layers.Flatten(),
                                keras.layers.Dropout(drop),
                                keras.layers.Dense(40, activation=LR),
                    ...
                    # 평가 결과 저장
                    fSave = open('DL_WPCN_ver200519_01_result_' + str(problemNo) + '.txt', 'w')
