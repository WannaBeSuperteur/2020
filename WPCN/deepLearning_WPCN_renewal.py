...
if __name__ == '__main__':
    ...
    if readOrWrite == 1: # 맵 파일 쓰고 그 맵으로 트레이닝, 테스트
        ...
        for i in range(numTrain + numTest):
            initScreen_ = WPCN_helper_renewal.initScreen(size, False)
        ...
        for y in range(size):
            for x in range(size):
                ...
                for HAPtime in HAPtimeList:
                    throughput = WPCN_helper_renewal.getThroughput(wdList[i], [y, x], size, HAPtime, problemNo)
                    # if HAPtime > 0.645 and HAPtime < 0.655: print(throughput)
    ...
    print('')
    for i in range(numTrain):
        optiY_IS = round(Qlearning_deep_helper.invSigmoid(optiY[i], size-1), 1)
        optiX_IS = round(Qlearning_deep_helper.invSigmoid(optiX[i], size-1), 1)
        optiHT_IS = round(Qlearning_deep_helper.invSigmoid(optiHT[i], 1.0), 3)
            ...
            for dro in range(len(dropout)): # 각 dropout에 대하여 실험
                ...
                while True:
                    try:                  
                        ...
                        print('training...')
                        Qlearning_deep.deepQlearning(0, NN, op, lc, inputs, outputs, None, None, 'WPCNdeep', [epoc, None], None, None, False, False, True)
                        ...
                        for i in range(numTest):
                            ...
                            # 6-0. 테스트 결과 확인
                            # 출력값 받아오기
                            optiY_test = Qlearning_deep_helper.invSigmoid(testOutputLayer[0][0], size-1) # 테스트 결과로 나온 throughput 최대화 y좌표
                            optiX_test = Qlearning_deep_helper.invSigmoid(testOutputLayer[0][1], size-1) # 테스트 결과로 나온 throughput 최대화 x좌표
                            optiHT_test = Qlearning_deep_helper.invSigmoid(testOutputLayer[0][2], 1.0) # 테스트 결과로 나온 throughput 최대화 충전시간
                            ...
                            # 테스트 결과 받아오기
                            throughput = WPCN_helper_renewal.getThroughput(wdList[numTrain + i], [optiY_test, optiX_test], size, optiHT_test, problemNo)
                            ...
                            for y in range(size):
                                for x in range(size):
                                    ...
                                    for HAPtime in HAPtimeList:
                                        thisThroughput = WPCN_helper_renewal.getThroughput(wdList[numTrain + i], [y, x], size, HAPtime, problemNo)
