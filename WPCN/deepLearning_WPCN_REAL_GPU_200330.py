    ...
    for i in range(numTrain):
        ...
        for j in range(size):
            for k in range(size):
                ...
                outputScreen[i][j][k] = pow(outputScreen[i][j][k] + 1.0, 3) # uniform [0, 1] -> f(x) = x^3 from 1 to 2
                outputScreen[i][j][k] = outputScreen[i][j][k] * (2/7) - (9/7) # f(x) = x^3 from 1 to 2 -> f(x) = 2x^3/7 - 9/7 from 1 to 2
        ...
        for epo in range(len(epoch)): # 각 epoch에 대하여
            for dro in range(len(dropout)): # 각 dropout에 대하여 실험
                ...
                while True:
                    ...
                    # 평가 결과 저장
                    fSave = open('DL_WPCN_ver200519_03_result_' + str(problemNo) + '.txt', 'w')
