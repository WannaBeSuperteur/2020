...
# y = (1 - e^-2x) / (1 + e^-2x)
# y = 2 / (1 + e^-2x) - 1
# x = 2 / (1 + e^-2y) - 1 (exchange x and y to derive inverse function)
# x + 1 = 2 / (1 + e^-2y)
# 1 / (x + 1) = (1 + e^-2y) / 2
# 2 / (x + 1) = 1 + e^-2y
# 2 / (x + 1) - 1 = e^-2y
...
if __name__ == '__main__':
    ...
    for i in range(numTrain):
        ...
        for j in range(size):
            for k in range(size):
                outputScreen[i][j][k] /= maxVal # uniform [0, maxVal] -> uniform [0, 1]
                outputScreen[i][j][k] = outputScreen[i][j][k] * 1.5 - 0.75 # uniform [0, 1] -> uniform [-0.75, 0.75]

        # 테스트 시 역tanh를 적용할 것이므로 먼저 outputScreen의 모든 원소에 tanh를 적용
        for j in range(size):
            for k in range(size): outputScreen[i][j][k] = tanh(outputScreen[i][j][k])
                ...
                while True:
                    try:                  
                        ...
                        NN = [tf.keras.layers.Reshape((size, size, 1), input_shape=(size*size,)),
                                ...
                                keras.layers.Dense(size*size, activation='tanh'),
                        ...
                        for i in range(numTest):
                            ...
                            for j in range(size):
                                for k in range(size):
                                    optiScreen[j][k] = invTanh(testOutputLayer[0][0][j][k])
                    ...
                    # 평가 결과 저장
                    fSave = open('DL_WPCN_ver200519_07_result_' + str(problemNo) + '.txt', 'w')
