...
def layers(img, option, model):
    ...
    # input the image into the model, get the output
        ...
        layer_values_OUT = XAI_function_OUT(img) # shape: (1, 1, N) where N=7
...
if __name__ == '__main__':
    ...
    # load ground truth
    groundTruth_file = 'signSpeedLimitTest_groundTruth.csv'
    groundTruth = np.array(pd.read_csv(groundTruth_file, index_col=0))

    # run XAI with test images : every 20th image
    for i in range(testRows // 20):
    
        # with 100% probability
        if random.random() < 1.0:
            XAI(False, i*20, test_input, model, groundTruth, verboseOutput=False, verboseXAI=False)
