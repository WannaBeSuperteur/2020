...
# 파일 반환
# height이 None이 아니면 2차원 배열의 집합, None이면 1차원 배열의 집합 (또는 1개의 2차원 배열)
def getDataFromFile(fn, height):
    result = []
    f = open(fn, 'r')
    fLines = f.readlines()
    fLen = len(fLines)
    ...
    if height == None: # 1차원 배열
        for i in range(fLen): # 각 줄이 1차원 배열에 해당

            # 해당 줄을 공백으로 구분
            thisLineSplit = fLines[i].split('\n')[0].split('\t')

            # 해당 줄을 모두 숫자로 변환
            # 마지막에 있는 ''인 항목을 제외하고 숫자로 변환 및 추가
            for j in range(len(thisLineSplit)):
                if thisLineSplit[j] == '': thisLineSplit.pop() # 마지막의 '' 항목 삭제
                else: thisLineSplit[j] = float(thisLineSplit[j])
            
            result.append(thisLineSplit) # 해당 줄을 최종 결과 배열에 추가

    else: # 2차원 배열
        for i in range(int(fLen / height)):
            
            _2dArray = [] # result에 추가할 2차원 배열
            for j in range(height): # 각 줄에 대하여
                
                # 해당 줄을 공백으로 구분
                thisLineSplit = fLines[i * height + j].split('\n')[0].split('\t')

                # 해당 줄을 모두 숫자로 변환
                for k in range(len(thisLineSplit)): thisLineSplit[k] = float(thisLineSplit[k])
                _2dArray.append(thisLineSplit)

            result.append(_2dArray) # 2차원 배열을 최종 결과에 추가
